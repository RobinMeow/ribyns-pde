...
indentify using lsblk
createa tx using ´fdisk /dev/<yourdisk>´
...
w -> to write all to disk (this will erase all data which was on it beforehand)

format them

mkfs.ext4 /dev/nvmen1n1p3
...
or 
KEYMAP=de
for qwerty or qwertz respectively
...
and gpu drivers, if pacman -S nvidia is for the 4080
...
pacman -S plasma-desktop (this is the minimal one, but still contains kwallet)
...
install kscreen which allows you to rearrange monitors and do some resolution settings etc.
