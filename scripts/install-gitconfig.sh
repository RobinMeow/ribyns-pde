...
git config --file "$PDE/.gitconfig" --get-regexp '^(core|advice|pull|push)\.' | while read -r key value; do
...
# re-add them all
git config --file "$PDE/.gitconfig" --get-regexp '^alias\.' | while read -r key value; do
	key="${key#alias.}"
